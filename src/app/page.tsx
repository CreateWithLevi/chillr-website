...
export default function Home() {
  ...
  const chairsDataSec3 = [
    { src: "/sec3_chair1.svg", alt: "Chillr chair sketch 1", number: "1", detailImageSrc: "/sec3_img_chair1.jpg", title: "Pink Chaise Lounge", description: "Soft, elegant, and unapologetically unique. This pink chaise invites you to relax in its sculpted form, blending comfort and charm under a spotlight of beauty." },
    ...
    { src: "/sec3_chair3.svg", alt: "Chillr chair sketch 3", number: "3", detailImageSrc: "/sec3_img_chair3.jpg", title: "Black Lounge Chair", description: "Sink into modern sophistication. With its sleek, glossy frame and cushioned comfort, this chair wraps you in quiet luxury—perfect for unwinding in style." },
  ];
  ...
  return (
    <main className="flex flex-col min-h-screen bg-black text-white font-sans">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-black/30 backdrop-blur-md shadow-lg py-3 px-6 sm:px-8 md:px-16 lg:px-24 flex justify-between items-center">
        ...
        <button className="border border-yellow-300 text-yellow-300 px-5 py-2 rounded-full hover:bg-yellow-400 hover:text-black transition-colors text-sm font-medium absolute right-18 md:relative md:right-0">
          Chair Together
        </button>
          ...
          <div className="absolute top-12 left-10 max-w-[250px] md:max-w-[unset] md:top-auto md:bottom-12 left-8 right-8 md:grid sm:grid-cols-2 gap-6 text-sm sm:text-base font-bold">
            ...
            <div>
              {activeLeftImageSec3 !== "/sec3_img.jpg" && activeChairDetailsSec3.title ? (
                <h3 className="text-lg font-semibold mb-2">{activeChairDetailsSec3.title}</h3>
              ) : (
            ...
            <div>
              {activeLeftImageSec3 !== "/sec3_img.jpg" && activeChairDetailsSec3.description ? (
                <p>{activeChairDetailsSec3.description}</p>
              ) : (
